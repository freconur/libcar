...
const DAILY_SALE = "K92gVBfS5UyNkklUFkcZ"
const DB_VENTAS = "F24ZZQrIsBnLWUsh4MiH"
const PRODUCT_SALES = "product-sales"
const SALES_PER_MONTH = "veuXDaCqTdBjrN3EXPip"
const TICKET = "r92eRKXI51Z7iDMj6r3T"
...
export const generateSold = async (dispatch: (action: any) => void, cart: ProductToCart[] | undefined, cero: number) => {
  ...
  if(findProductAmountCero) {
    ...
    await setDoc(doc(db, `/salesPerMonth/${SALES_PER_MONTH}/mi-negocio/${currentYear()}/month-${currentYear()}/${currentMonth()}`), { month: `${currentMonth()}`, totalSales: 0 }) // debo verificar de donde obtengo la data de estadisticas antes de agregar o quitar esta data del endpoint
    ...
    await addProductFromCartToTicket(
      //pendiente modificar el atributo library18 que es un boleano
      {
